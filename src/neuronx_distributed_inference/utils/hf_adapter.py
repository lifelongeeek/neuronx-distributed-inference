...
class HuggingFaceGenerationAdapter(PreTrainedModel, GenerationMixin):
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # Conditionally pass tensor_capture_hook only to models that accept it (e.g., multimodal).
        # Text-only models like GLM-4.5 MoE do not declare this parameter.
            ...
            if return_dict_in_generate:
                if output_scores:
                    ...
                    if self.capture_draft_logits:
                        ...
                        scores += tuple(
                            outputs.fused_outputs[-1][:, i, :] for i in range(n_matches)
                        )
